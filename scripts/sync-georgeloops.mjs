...
function validateSources(sources) {
  for (const source of sources) {
    if (source.catalogUrl) {
      throw new Error(
        `${source.label || source.repo} uses catalogUrl; Loops Radar sources must be GitHub repo files.`,
      );
    }

    if (!source.repo || !source.branch || !Array.isArray(source.include)) {
      throw new Error(
        `${source.label || source.repo || "Unknown source"} must define repo, branch, and include patterns.`,
      );
    }
  }
}
